function [LF,FDL,D]=FDL_function(Infile,name,numLayers)
...
LF=zm_loadLF(Infile,name,uRange,vRange);
LF1 = double(reshape(LF,size(LF,1),size(LF,2),size(LF,3),[]))/255;
imgSize = [size(LF1,1),size(LF1,2)];
nChan = size(LF1,3);
if(linearizeInput)
    LF1 = BT709_gammaDecode(LF1+gammaOffset);
end
...
for l=1:numInitViews
    ...
    if(useGPU)
        orgPad = gpuArray(padarray(single(LF1(:,:,:,idView)),[padSizeY padSizeX],padType));
    else
        orgPad = padarray(single(LF1(:,:,:,idView)),[padSizeY padSizeX],padType);
    end
    if(Windowing)
